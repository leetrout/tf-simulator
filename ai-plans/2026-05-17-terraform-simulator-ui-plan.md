...
Start with a backend/frontend foundation PR rather than the full graph editor:

1. Add an `internal/simulator` package with Terraform state types, graph DTO types, and conversion functions from current store state.
2. Add `GET /api/sim/state`, `GET /api/sim/terraform-state`, `GET /api/sim/api-resources`, and `GET /api/sim/graph`.
3. Add typed frontend API client and Svelte stores.
4. Replace websocket hardcoding with a dynamic host.
5. Add tests for graph and state generation.

This creates stable data contracts before building the polished graph and JSON editor UI.
