...
// A scratch context for text measurement (no DOM layout needed for sizing).
...
// Rasterise one node to a bitmap at the given pixel ratio.
export function rasterizeTable(t, theme, dpr) {
    ...
    ctx.scale(dpr, dpr);
    const r = 10;
    // statesim patch: per-node status accent read from the adapter's data
    // passthrough (set by the GraphPane). Falls back to the global theme.
    const accent = (t.data && typeof t.data === "object" && t.data.statusColor) || null;
    // body
    roundRect(ctx, 0.5, 0.5, w - 1, h - 1, r);
    ctx.fillStyle = theme.tableBg;
    ctx.fill();
    // statesim patch: left status stripe inside the rounded body
    if (accent) {
        ctx.save();
        roundRect(ctx, 0.5, 0.5, w - 1, h - 1, r);
        ctx.clip();
    ...
    // statesim patch: tint the header with the status accent
    if (accent) {
    ...
    // border (statesim patch: use the status accent when present)
    roundRect(ctx, 0.5, 0.5, w - 1, h - 1, r);
    ctx.strokeStyle = accent || theme.tableBorder;
...
// y-position (node-local) of a column's connection point.
