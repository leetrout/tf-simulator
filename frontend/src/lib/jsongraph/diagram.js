...
export class Diagram {
    constructor(canvas) {
        this.viewW = 0;
        this.viewH = 0;
        // event callbacks (assign these or pass them via createGraph)
        this.onHiddenChange = null;
        ...
        this.model = { tables: [], relations: [], errors: [] };
        this.themeName = "dark";
        ...
        this.editing = null;
        this.editable = true; // master switch: inline edits, "+ add column", and new connections
    ...
    setModel(model, { keepCamera = false } = {}) {
        // preserve positions of nodes that still exist (so live edits don't jump)
    ...
    // The node whose relationships should be emphasised: a click-pinned node
    ...
    _setPin(t) {
        ...
        for (const r of this.model.relations) {
            if (r.fromTable.toLowerCase() === t.key)
                keys.add(r.toTable.toLowerCase());
            if (r.toTable.toLowerCase() === t.key)
                keys.add(r.fromTable.toLowerCase());
        }
    ...
    _render() {
        ...
        for (const t of this.model.tables) {
            if (!Number.isFinite(t.x) || this.hidden.has(t.key))
                continue;
            if (t.x > vx1 + margin || t.x + t.w < vx0 - margin || t.y > vy1 + margin || t.y + t.h < vy0 - margin)
                continue;
        ...
        if (pinned && Number.isFinite(pinned.x) && this.editable)
            this._drawAddButton(pinned);
        for (const a of this.annotations)
            if (a.type === "note")
                this._drawNote(a, cull);
        if (this.selectedAnno)
            this._drawAnnoChrome(this.selectedAnno);
        if (this.marquee) {
            const m = this.marquee;
            const x = Math.min(m.ax, m.x), y = Math.min(m.ay, m.y);
            const w = Math.abs(m.x - m.ax), h = Math.abs(m.y - m.ay);
            ctx.save();
            ctx.fillStyle = hexA(theme.edgeHi, 0.1);
            ctx.strokeStyle = theme.edgeHi;
        ...
        if (this.editable && this.hoverConn && !this.linking && !this.drag && !this.dragGroup && !this.pan) {
    ...
    _drawAnnoChrome(a) {
        ...
        const k = del.r * 0.45;
        ctx.beginPath();
        ctx.moveTo(del.cx - k, del.cy - k);
        ctx.lineTo(del.cx + k, del.cy + k);
        ctx.moveTo(del.cx + k, del.cy - k);
        ctx.lineTo(del.cx - k, del.cy + k);
        ctx.stroke();
    }
    ...
    _annoChromeAt(sx, sy) {
        ...
        for (const d of rects.colors) {
            if ((w.x - d.cx) ** 2 + (w.y - d.cy) ** 2 <= (d.r * 1.4) ** 2)
                return { kind: "color", value: d.key };
        }
        const del = rects.delete;
        if ((w.x - del.cx) ** 2 + (w.y - del.cy) ** 2 <= (del.r * 1.4) ** 2)
            return { kind: "delete" };
        const rz = rects.resize;
        if (w.x >= rz.x - 4 / this.cam.scale &&
            w.x <= rz.x + rz.w + 4 / this.cam.scale &&
            w.y >= rz.y - 4 / this.cam.scale &&
            w.y <= rz.y + rz.h + 4 / this.cam.scale)
            return { kind: "resize" };
        return null;
    }
    _beginEditAnnotation(a) {
        ...
        el.select();
        const commit = () => {
            if (!this.editing || this.editing.anno !== a)
                return;
            a.text = el.value;
            this.editing = null;
            el.remove();
        ...
        // el is a textarea|input union, so the typed-event-map overload doesn't
        // apply — take a generic Event and narrow to KeyboardEvent.
    ...
    _addButtonAt(sx, sy) {
        if (!this.editable || !this.pinned || !Number.isFinite(this.pinned.x))
            return false;
        const w = this.screenToWorld(sx, sy);
    ...
    _edgeSeg(fromKey, fromCol, toKey, toCol, cull) {
        ...
        if (!from || !to || !Number.isFinite(from.x) || !Number.isFinite(to.x))
            return null;
        if (this.hidden.has(from.key) || this.hidden.has(to.key))
            return null;
        ...
        if (cull) {
            ...
            if (maxX < cull.x0 || minX > cull.x1 || maxY < cull.y0 || minY > cull.y1)
                return null;
        }
    ...
    _drawEdges(vx0, vy0, vx1, vy1) {
        ...
        const fadeAlpha = this.pinned ? 0.05 : 0.16;
        const highlighted = [];
        const edges = [];
        for (const r of this.model.relations)
            edges.push({ fk: r.fromTable.toLowerCase(), tk: r.toTable.toLowerCase(), fc: r.fromCols[0], tc: r.toCols[0], manual: false });
        for (const l of this.manualLinks)
            edges.push({ fk: l.from.table, tk: l.to.table, fc: l.from.col, tc: l.to.col, manual: true });
        ...
        for (const seg of highlighted)
            this._stroke(seg, theme.edgeHi, 2.2, 1, seg.manual);
    }
    ...
    _bindInput() {
        ...
        window.addEventListener("mousemove", (e) => {
            ...
            // connector dots on the hovered column row (only when connections allowed)
            let conn = null;
            if (t && this.editable) {
                const w = this.screenToWorld(sx, sy);
        ...
        c.addEventListener("touchstart", (e) => {
            ...
            if (e.touches.length === 1) {
                ...
                this.drag = this.pan = this.annoDrag = this.annoResize = null;
                tap = null;
        ...
        c.addEventListener("touchmove", (e) => {
            if (pinch && e.touches.length >= 2) {
                ...
                this.cam.x = mx - (mx - this.cam.x) * k;
                this.cam.y = my - (my - this.cam.y) * k;
                this.cam.x += mx - pinch.mx;
                ...
                const p = tpos(e.touches[0]);
                if (Math.abs(p.sx - tap.sx) + Math.abs(p.sy - tap.sy) > 8)
                    tap.moved = true;
    ...
    _pointerDown(sx, sy, additive = false, allowConnect = true) {
        ...
        if (this._addButtonAt(sx, sy)) {
            this.onAddColumn?.(this.pinned.key);
            return;
        }
        const note = this._noteAt(sx, sy);
        if (note) {
            this._grabAnno(note, sx, sy);
            return;
        }
        const t = this.tableAt(sx, sy);
        if (t) {
    ...
    _pointerMove(sx, sy) {
        ...
        if (this.pan) {
            ...
            if (Math.abs(sx - this.pan.sx) + Math.abs(sy - this.pan.sy) > 3)
                this.pan.moved = true;
            this.markDirty();
            return true;
        }
    ...
    _pointerUp() {
        if (this.linking) {
            ...
            if (tgt)
                this.addManualLink(k.fromKey, k.fromCol, tgt.tableKey, tgt.col);
            this.markDirty();
            return;
        }
        if (this.marquee) {
        ...
        if (this.drag && !this.drag.moved)
            ...
            if (this.pinned)
                this._pin(this.pinned);
        }
        const changed = (this.drag && this.drag.moved) ||
            (this.pan && this.pan.moved) ||
    ...
    hideTable(t) {
        let keys;
        if (t && this.selected.has(t) && this.selected.size > 1)
            keys = [...this.selected].map((x) => x.key);
        else if (t)
            keys = [t.key];
        else
            keys = [...this.selected].map((x) => x.key);
    ...
    _connectorAt(sx, sy) {
        if (!this.editable)
            return null; // new connections disabled when not editable
        const w = this.screenToWorld(sx, sy);
    ...
    addManualLink(fk, fc, tk, tc) {
        if (fk === tk && fc.toLowerCase() === tc.toLowerCase())
            return false;
        if (this._linkExists(fk, fc, tk, tc))
            return false;
    ...
    inferLinks() {
        ...
        for (const l of this.manualLinks)
            seen.add(ek(l.from.table, l.from.col, l.to.table, l.to.col));
        const added = [];
        for (const t of tables) {
            for (const c of t.columns) {
                if (c.pk)
                ...
                if (tk && tc && !seen.has(ek(t.key, c.name, tk, tc)) && !seen.has(ek(tk, tc, t.key, c.name))) {
    ...
    _editAt(sx, sy) {
        ...
        if (!t) {
            ...
            if (anno) {
                ...
                this._beginEditAnnotation(anno);
                this.markDirty();
                return;
            }
        }
    ...
    _editTargetAt(sx, sy) {
        ...
        if (w.x >= split && col.type) {
            return {
                ...
                colName: col.name,
                value: col.type,
                rect: { x: split, y: rowY, w: t.x + t.w - split, h: ROW_H },
                align: "right",
                weight: 400,
            };
        }
    ...
    _beginEdit(target) {
        ...
        input.style.fontWeight = String(target.weight);
    ...
    fit(padding = 60) {
        ...
        const scale = clamp(Math.min((this.viewW - padding * 2) / bw, (this.viewH - padding * 2) / bh), 0.08, 1.5);
...
function inside(w, a) {
    return w.x >= a.x && w.x <= a.x + a.w && w.y >= a.y && w.y <= a.y + a.h;
}
function hexA(hex, a) {
...
function clip(ctx, text, maxW) {
    if (ctx.measureText(text).width <= maxW)
        return text;
    let s = text;
    while (s.length && ctx.measureText(s + "…").width > maxW)
        s = s.slice(0, -1);
    return s + "…";
}
